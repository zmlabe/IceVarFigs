...
datasets = ['GISTEMP','Berkeley','ERA20C','JRA55','ERAi']
...
color=iter(cmocean.cm.haline(np.linspace(0.,1,len(datasets))))
for i in range(len(datasets)):
    c=next(color)
...
l = plt.legend(shadow=False,fontsize=7,loc='upper center',
           bbox_to_anchor=(0.5, 1.02),fancybox=True,ncol=5,frameon=False)
...
a = plt.axes([.35, .59, .29, .24], axisbg='w') 
